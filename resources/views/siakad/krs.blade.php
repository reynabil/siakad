...
<body>
    <h1 class="text-center mb-4">DATA KRS</h1>

    <div class="container">
        <a href="/tambahdatakrs" class="btn btn-success mb-5">Tambah Data</a>
        <div class="row">
            ...
            <table class="table table-bordered" id="example">
                ...
                <tbody>
                    ...
                    @foreach ($data as $row)
                        <tr>
                            <th scope="row">{{ $no++ }}</th>
                            <td>{{ $row->matkuls->matkul}}</td>
                            <td>{{ $row->kode_matkul}}</td>
                            <td>{{ $row->mahasiswas->nama_mahasiswa }}</td>
                            <td>{{ $row->nim}}</td>
                            <td>{{ $row->jurusans->jurusan}}</td>
                            <td>{{ $row->kode_jurusan}}</td>
                            <td>
                                <a href="/tampildatakrs/{{ $row->id }}" class="btn btn-warning"><i class="fa-sharp fa-solid fa-pen-to-square"></i></a>
                                <a href="#" class="btn btn-danger delete" data-id="{{ $row->id }}"
                                    data-nama_mahasiswa="{{ $row->nama_mahasiswa }}"><i class="fa-sharp fa-solid fa-trash"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
...
<script>
    $('.delete').click(function() {
        ...
        var nama = $(this).attr('data-nama_mahasiswa');
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    window.location = "/deletekrs/" + mahasiswaid + ""
